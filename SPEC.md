...
| # | 項目 | 決定内容 |
|---|---|---|
| 1 | 料金の下限 | 30分あたり 300 円（最低利用枠数によらず一律）。`PRICING.minPricePer30min` |
| 2 | キャンセル回数に含める範囲 | 利用者自身のキャンセルはすべて数える（利用開始後の返金なし、6回目以降の返金なしも含む）。貸出主・運営のキャンセルと no_show は含めない |
| 3 | 予約受付期間（30日）の判定 | 日付で判定する。利用日（Asia/Tokyo）が「今日 + 30日」以下なら受け付ける |
| 4 | 日をまたぐ予約 | 受け付けない。日をまたいで利用したい場合は日ごとに別々に予約する（24:00 ちょうどに終わる予約は可） |
| 5 | 「11. フェーズ」の参照 | 「13. フェーズ」のこととする |
